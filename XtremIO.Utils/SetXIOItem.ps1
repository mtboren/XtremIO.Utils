...
# 	.Example
# 	Set-XIOSnapshotScheduler -SnapshotScheduler (Get-XIOSnapshotScheduler mySnapshotScheduler0) FINISH HERE
# 	Set the given SnapshotScheduler to FINISH HERE
# 	.Example
# 	Get-XIOSnapshotScheduler -Name mySnapshotScheduler0 -Cluster myCluster0 -ComputerName somexms.dom.com | Set-XIOSnapshotScheduler FINISH HERE
# 	Get the given SnapshotScheduler from the specified cluster managed by the specified XMS, and set FINISH HERE
# 	.Outputs
...
# 		[parameter(ParameterSetName="ByTimespanInterval_SpecifySnapNum")]
# 		[parameter(ParameterSetName="ByTimespanInterval_SpecifySnapAge")][ValidateScript({$_ -le (New-TimeSpan -Hours 72)})][System.TimeSpan]$Interval,
...
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapNum")]
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapAge")]
...
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapNum")]
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapAge")][System.DateTime]$ExplicitTimeOfDay,
...
# 		[parameter(ParameterSetName="ByTimespanInterval_SpecifySnapNum")]
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapNum")][ValidateRange(1,511)][int]$SnapshotRetentionCount,
...
# 		[parameter(ParameterSetName="ByTimespanInterval_SpecifySnapAge")]
# 		[parameter(ParameterSetName="ByExplicitSchedule_SpecifySnapAge")]
# 		[ValidateScript({($_ -ge (New-TimeSpan -Minutes 1)) -and ($_ -le (New-TimeSpan -Days (5*365)))})][System.TimeSpan]$SnapshotRetentionDuration,



# 		## Switch:  Snapshot Scheduler enabled-state. To enable the SnapshotScheduler, use -Enable.  To disable, use -Enable:$false. When disabling, can apparently not make other changes, as the API uses a different method in the backend on the XMS ("suspend_scheduler" instead of "modify_scheduler")
# 		[Switch]$Enable,
...
# 		Switch($PsCmdlet.ParameterSetName) {
# 			{"ByTimespanInterval_SpecifySnapNum", "ByExplicitSchedule_SpecifySnapNum" -contains $_} {$hshSetItemSpec["snapshots-to-keep-number"] = $SnapshotRetentionCount; break}
# 			{"ByTimespanInterval_SpecifySnapAge", "ByExplicitSchedule_SpecifySnapAge" -contains $_} {$hshSetItemSpec["snapshots-to-keep-time"] = [System.Math]::Floor($SnapshotRetentionDuration.TotalMinutes)}
# 		} ## end switch
...
# 		if ($PSBoundParameters.ContainsKey("SnapshotRetentionCount")) {$hshSetItemSpec["snapshots-to-keep-number"] = $SnapshotRetentionCount}
# 		if ($PSBoundParameters.ContainsKey("Enable")) {$hshSetItemSpec["state"] = $(if ($Enabled) {'enabled'} else {'user_disabled'})}
